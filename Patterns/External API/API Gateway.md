...
## Related patterns
* API Gateway使用[客户端服务发现模式](/Patterns/Service discovery/Client-side discovery.md)（Client-side Discovery pattern）或者服务端服务发现模式（Server-side Discovery pattern）路由请求到可用的服务实例。
